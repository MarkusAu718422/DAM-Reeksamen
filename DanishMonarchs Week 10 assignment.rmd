...
```
Answer: The first two options gives us a Fair overview of the dataset, wherein number two though also gives us a R Console window. The next two gives us unfiltered text like it would be found in the excell arc number four also gives us an r console
We are also told which delimiter is needed for each command.



1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object? - Glimpse
2. What is the result of running `class()` on the `kings` object created with a tidyverse function. It showcases that the Kings object is a data.frame
3. How many columns does the object have when created with these different functions? View(kings) shows us 12 columns whereas Glimpse tells us of 11 columns and 57 rows, which also means we don't have to count it ourselves
...
kings <- read.csv("C:/Users/Bruger/Documents/Rstudio fis/Week 10 assignment/kings.csv")
