...
Content_Athenae <- paste0("Athens came under roman influence in the 2nd century and was incorporated in 146 BC, only halted by a revolt in 88 BC, this led to a devastating siege on the city which ended in 86 BC which destroyed a lot of monuments. Even though athens had been opposed to roman rule, The romans were still heavily inspired by the Greek culture which was most prominent in Athens. Therefore it became a significant cultural and Educational hub for the Romans and others.
...
Content_Barcino <- paste0("The Romans founded this colony between 15-13 BC  and had no trouble gaining control over the area, as many other colonies, it grew rapidly and walls were built around the now city between the 1st and 2nd century AD, and it was later reinforced between the 3rd and 4th century. The city was built like a typical Roman city, with a forum in the middle and two big streets.
...
Content_Antiochia <- paste0("Antioch was annexed by the Romans in 64 BC and made it the capital of the province of Syria, the city was already flourishing from the Secluid rule, and the roman occupation only saw it fit to continue the beauty of the city. It became a major commercial hub, and was later referred to as “the queen of the east”, and its location along the silk road only helped the prosperity of the city. Though it was the victim of countless attacks from Persians, it joined the byzantine empire during the split. it stood strong until a series of unfortunate events took place, fires, earthquakes and plagues, led to its eventual capture by the arabs in the mid 7th century",
...
#        #
data <- data.frame(
  lng = c(23.726559, 22.879795, 29.903728, 10.327685, 14.29306, -4.78025, 4.820576,
          1.25763, 12.485098, 9.188141, 22.94554, 36.169629, 35.235357, 28.968935,
          22.5, 22.432584, 23.317799, 23.84013, 31.255061, 32.600128, 13.187507, 14.5,
          21.854404, 34.892717, 27.341752, 30.15853, -5.996676, 2.147368,
          -0.878491, -6.292409, -1.082286, -0.086632, 12.798272, 12.629052,
          14.251636, 13.361376, 10.40074, -0.574367, 5.364897, 4.878056, 6.959912,
          8.277769, 10.894765, 35.50237, 36.204326, 35.815212, 35.194564, 36.306587,
          36.482118, 34.893976, 35.442275, 32.406593, 33.902516, 23.572665, 16.48323,
          -8.42699, -8.42699, 21.893884, 14.366944, 6.2667, 19.050216, 23.342891, 19.456811,
          8.708877, 6.139959, 18.767302, 17.437348, 5.466667, 5.858672, 21.342, 21.195698,
          15.499078, 14.402948, 21.391437),
  lat = c(37.971119, 37.906321, 31.195024, 36.851318, 32.63833, 37.88376, 45.759119, 41.117936,
          41.892777, 45.463689, 40.637747, 36.210172, 31.777766, 41.010111, 38.483333,
          37.074301, 38.319077, 40.821057, 29.849491, 31.049917, 32.876174, 32.5, 32.820182,
          36.917425, 37.938309, 38.076044, 37.394112, 41.349108, 41.654027, 36.529942,
          53.957702, 51.510879, 41.963579, 41.447947, 40.840097, 38.115619, 43.714368,
          44.839577, 43.303884, 43.303884, 50.940664, 49.993057, 48.365463, 33.892408,
          34.006966, 34.448685, 33.271637, 33.511647, 32.518318, 31.617141, 30.328981,
          34.757212, 35.1848, 46.069003, 43.539485, 41.550136, 43.219135, 43.316295, 46.69775,
          35.46081, 47.56473, 42.722826, 41.327377, 36.174527, 46.20381, 42.421537, 44.053454,
          50.783333, 51.84167, 41.011, 42.600105, 45.920942, 35.885912, 42.016885),
  value = rep(2, 74),
  Cities = c("Athenae", "Corinthia", "Alexandria", "Carthago", "Lepcis Magna", "Corduba",
             "Lugdunum", "Tarraco", "Roma", "Mediolanum", "Thessalonica", "Antiochia",
             "Aelia Capitolina", "Byzantium", "Delphi", "Sparta", "Thebae", "Amphipolis",
             "Memphis", "Pelusium", "Oea", "Tacapae", "Cyrene", "Tarsus", "Ephesus", "Celaenae",
             "Hispalis", "Barcino", "Caesaraugusta", "Gades", "Eburacum", "Londinium", "Tibur",
             "Antium", "Neapolis", "Panormus", "Pisae", "Burdigala", "Massalia", "Vienna",
             "Colonia Agrippinensis", "Mogontiacum", "Augusta Vindelicum", "Berytus",
             "Heliopolis (Syria)", "Tripolis", "Tyrus", "Damascus", "Bostra", "Gaza", "Petra",
             "Neapaphus", "Salamis", "Apulum", "Salona", "Bracara", "Marcianopolis", "Naissus",
             "Virunum", "Lambaesis", "Aquincum", "Serdica", "Dyrrachium", "Sicca Veneria",
             "Genava", "Acruvium", "Bistua Nova", "Atuatuca", "Noviomagus (Germania Inferior)",
             "Heraclea (Macedonia)", "Ulpiana", "Neviodunum", "Melita", "Scupi")
)


data$groups <- data$Cities
...
my_map <- leaflet(data) %>%
  ...
  addMarkers(lng = 12.485098, lat = 41.892777, group = "Roma", popup = Content_Roma) %>%
  addMarkers(lng = 14.4871, lat = 40.7497, group = "Pompeii", popup = Content_Pompeii) %>%
  addMarkers(lng = 29.903728, lat = 31.195024, group = "Alexandria", popup = Content_Alexandria) %>%
  addMarkers(lng = 36.169629, lat = 36.210172, group = "Antiochia", popup = Content_Antiochia) %>%
  addMarkers(lng = 28.968935, lat = 41.010111, group = "Byzantium", popup = Content_Byzantium) %>%
  addMarkers(lng = -0.086632, lat = 51.510879, group = "Londinium", popup = Content_Londinium) %>%
  addMarkers(lng = 10.327685, lat = 36.851318, group = "Carthago", popup = Content_Carthago) %>%
  addMarkers(lng = 27.341752, lat = 37.938309, group = "Ephesus", popup = Content_Ephesus) %>%
  addMarkers(lng = 23.726559, lat = 37.971119, group = "Athenae", popup = Content_Athenae) %>%
  addMarkers(lng = -0.878491, lat = 41.654027, group = "Caesaraugusta", popup = Content_Caesaraugusta) %>%
  addMarkers(lng = 2.147368, lat = 41.349108, group = "Barcino", popup = Content_Barcino) %>%
  addMarkers(lng = 6.959912, lat = 50.940664, group = "Colonia Agrippinensis", popup = Content_Colonia_Agrippinensis) %>%
  addMarkers(lng = 19.050216, lat = 47.56473, group = "Aquincum", popup = Content_Aquincum) %>%
  addMarkers(lng = 21.893884, lat = 43.316295, group = "Naissus", popup = Content_Naissus) %>%
  addMarkers(lng = 35.50237, lat = 33.892408, group = "Berytus", popup = Content_Berytus) %>%
  addMarkers(lng = 14.29306, lat = 32.63833, group = "Lepcis Magna", popup = Content_Lepcis_Magna) %>%
  ...
  addMarkers(lng = -5.996676, lat = 37.394112, group = "Iulia_Romula_Hispalis", popup = Content_Iulia_Romula_Hispalis) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = unique(data$groups),
...
browsable(
  tagList(
    title_html,
    div(style = "width: 100%; aspect-ratio: 16 / 9;", my_map)
  )
)        
